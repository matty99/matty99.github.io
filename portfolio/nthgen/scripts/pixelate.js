...
function pixelate() {
	...
	$('.member-photo img').each(function(){
		...
		.css({"position":"absolute","z-index":"5","opacity":"0"})
		.insertBefore(el)
		.queue(function(){
			var el = $(this);
			el.parent().css({
	...
	$('.member-photo').mouseover(function(){
		if ($(window).width() > 1049) $(this).parent().find('img:first').stop().animate({opacity:1}, 600);
	})
	$('.img_grayscale').mouseout(function(){
		if ($(window).width() > 1049) $(this).stop().animate({opacity:0}, 600);
	});
}
